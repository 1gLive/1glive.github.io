...
function hidePlayer(e) {
	console.log("hidePlayer: error_msg=" + e);
	var t = "",
	i = "",
	a = "";
	if ("" !== e) {
		clearInterval(geoCheckTimer);
		var n = playerContainer.find(".vdo-panel");
		"en" == hpvars.lang ? (i = "Your network has been disconnected", a = "Retry") : (i = "您的網絡已中斷", a = "重試"),
		t = '<div class="upper"><div class="warning-desc failed">' + i + '</div><hr><div class="lower"><a class="common_btn2 continue_btn back-home" href="">' + a + '</a></div><div class="errorCode">[' + e + "]</div></div>",
		void 0 === n[0] ? playerContainer.append('<div class="vdo-panel"><div class="warning-panel">' + t + "</div></div>") : $("#videoContainer .vdo-panel .warning-panel").html(t);
		try {
			videoElement.pause(),
...
function initPlayer() {
	if (printLog("initPlayer:: "), $(".adContainer").remove(), $(".epg-right").remove(), $(".epg-left").css("width","945px"), $("#playPauseButton").css({"display":"block", "position":"relative", "width":"32px", "height":"28px", "margin":"5px 2px"}), apiInfo.profiles.hasOwnProperty("auto") ? defaultQuality = "auto": apiInfo.profiles.hasOwnProperty("hd") ? defaultQuality = "hd": apiInfo.profiles.hasOwnProperty("high") ? defaultQuality = "high": apiInfo.profiles.hasOwnProperty("low") && (defaultQuality = "low"), "live" == hpvars.videoType) initUrl = apiInfo.profiles[defaultQuality],
	...
	0 === hpvars.timeLimit && (seekBar.style.display = "none", currentTime.style.display = "none");
		...
		void 0 !== apiInfo.subtitle && Object.keys(apiInfo.subtitle).length > 0 ? (defaultSubtitleLang = Object.keys(apiInfo.subtitle)[0], defaultSubtitle = apiInfo.subtitle[defaultSubtitleLang], initSubtitle = defaultSubtitle.replace("web_subtitle", "web_subtitle_cors")) : (apiInfo.subtitle = {
			auto: "https://api.ttml.mytvsuper.tvb.com/web_subtitle_default.php"
		},
	...
	delete(qualityList = apiInfo.profiles).auto,
...
function onError(e) {
	...
	"6007" == e.code ? slaveExpiredRedirect("HP01-" + e.code) : 1 != e.category && 6 != e.category || (hidePlayer("HP01-" + e.code), video.pause())
}
var haveSurveyCompleted, instreamSurveyId, ads_volume_level = 1;
function setAdsVolume(e) {
	ads_volume_level = e
}
...
function requestAds() {
	...
	(adsLoader = new google.ima.AdsLoader(adDisplayContainer)).addEventListener(google.ima.AdsManagerLoadedEvent.Type.ADS_MANAGER_LOADED, onAdsManagerLoaded, !1),
	adsLoader.addEventListener(google.ima.AdErrorEvent.Type.AD_ERROR, onAdError, !1);
	var e = new google.ima.AdsRequest;
	e.adTagUrl = hpvars.adTag,
	e.linearAdSlotWidth = 640,
	e.linearAdSlotHeight = 360,
	e.nonLinearAdSlotWidth = 640,
	e.nonLinearAdSlotHeight = 360,
	adsLoader.requestAds(e)
}
function contentEndedListener() {
	void 0 !== adsLoader && adsLoader.contentComplete()
}
function onAdsManagerLoaded(e) {
	var t = new google.ima.AdsRenderingSettings;
	t.restoreCustomPlaybackStateOnAdBreakComplete = !0,
	(adsManager = e.getAdsManager(video, t)).addEventListener(google.ima.AdErrorEvent.Type.AD_ERROR, onAdError),
	...
	try {
		video.clientWidth <= 640 ? adsManager.init(640, 360, google.ima.ViewMode.NORMAL) : adsManager.init(video.clientWidth, video.clientHeight, google.ima.ViewMode.FULLSCREEN),
		adsManager.start()
	} catch(e) {
		video.play()
	}
...
function onAdEvent(e) {
	var t;
	switch (printLog("onAdEvent:: " + e.type), ad = e.getAd(), e.type) {
	case google.ima.AdEvent.Type.LOADED:
		ad.isLinear() || video.play();
		break;
	case google.ima.AdEvent.Type.STARTED:
		var i = remainingTime = 0,
		a = adsTime_sec = 0;
		if (ad.isLinear()) {
			adsManager.setVolume(ads_volume_level);
			var n = ad.getDuration();
			adsDuration_min = Math.floor(n / 60),
			adsDuration_sec = parseInt(n % 60),
			$("#adContainer").css("display", "block"),
			intervalTimer = setInterval(function() {
				remainingTime = adsManager.getRemainingTime(),
				i = n - remainingTime + 1,
				a = Math.floor(i / 60),
				adsTime_sec = parseInt(i % 60),
				countdownUi.innerHTML = langConvert("ads") + " (" + pad(a, 2) + " : " + pad(adsTime_sec, 2) + " / " + pad(adsDuration_min, 2) + " : " + pad(adsDuration_sec, 2) + ")",
				remainingTime <= 0 && (countdownUi.innerHTML = "", clearInterval(intervalTimer))
			},
			300)
		}
		hpvars.showFlip && $("#flipContainer").show();
		break;
	case google.ima.AdEvent.Type.SKIPPED:
		ad.isLinear() && (countdownUi.innerHTML = "", clearInterval(intervalTimer)),
		printLog("survey url= " + ad.getSurveyUrl()),
		(t = ad.getSurveyUrl()) && (instreamSurveyId = parseInt(t.replace(/[^\d]/g, "")), haveSurveyCompleted = survey.checkInvalidSurveyBeforeRender(instreamSurveyId));
		break;
	case google.ima.AdEvent.Type.COMPLETE:
		ad.isLinear() && (countdownUi.innerHTML = "", clearInterval(intervalTimer)),
		printLog("survey url= " + ad.getSurveyUrl()),
		(t = ad.getSurveyUrl()) && (printLog("survey id= " + (instreamSurveyId = parseInt(t.replace(/[^\d]/g, "")))), printLog("haveSurveyCompleted=" + (haveSurveyCompleted = survey.checkInvalidSurveyBeforeRender(instreamSurveyId))));
		break;
	case google.ima.AdEvent.Type.ALL_ADS_COMPLETED:
		video.ended ? !1 === haveSurveyCompleted ? video.pause() : hpvars.haveSurvey ? survey.videoEndCall(hpvars.nextVID) : "" != hpvars.nextVID ? nextEpURL(hpvars.nextVID) : playerCall.onVideoEnd() : video.play()
	}
}
function onAdError(e) {
	console.log("onAdError"),
	console.log(e),
	void 0 !== adsManager && adsManager.destroy(),
	$("#adContainer").css("display", "none"),
	hpvars.showFlip ? showFlip() : video.play()
}
function onContentPauseRequested() {
	...
	var e = 0;
	"object" == typeof ad && void 0 !== ad.getAdPodInfo().getPodIndex() && (e = ad.getAdPodInfo().getPodIndex()),
	e >= 0 && (video.pause(), $("#giantPlayButtonContainer").css("display", "none"), $("#adContainer").css("display", "block"), $("#controlsContainer").css("display", "none"))
}
function onContentResumeRequested() {
	...
	var e = 0;
	"object" == typeof ad && void 0 !== ad.getAdPodInfo().getPodIndex() && (e = ad.getAdPodInfo().getPodIndex()),
	0 === e ? (hpvars.showFlip ? (printLog("hpvars.showFlip"), showFlip()) : video.play(), player.isLive() && jumpToLive()) : e > 0 && video.play()
}
function jumpToLive() {
