...
var player, seekBar, currentTime, apiInfo, selectedAudioLang, selectedQulity, subtitleList, audioList, qualityList, contentId, subtitleLive, adsManager, adsLoader, adDisplayContainer, intervalTimer, video, ad, initUrl = "",
initSubtitle = "",
defaultAudioLang = "Cantonese",
defaultSubtitleLang = "tc",
defaultQuality = "auto",
...
function initApp() {
	...
	var e, t = hpconfig.api_domain;
	printLog("api_url= " + (e = "live" == hpvars.videoType ? "https://" + t + "/rest_app_user_subscription_api/video_checkout/format/jsonp?device_type=WEB_" + getBrowser() + "&type=channel&id=" + hpvars.channel_code + "&token=" + hpvars.token: "https://" + t + "/rest_app_user_subscription_api/video_checkout/format/jsonp?device_type=WEB_" + getBrowser() + "&type=video&id=" + hpvars.videoId + "&token=" + hpvars.token)),
	$.ajax({
		url: e,
		dataType: "jsonp",
		jsonp: "callback",
		...
		void 0 !== e.content ? 33445500 == e.content.error_code || 33556600 == e.content.error_code ? slaveExpiredRedirect(e.content.error_code) : (hidePlayer("HP02-" + e.content.error_code), printLog("error= HP02-" + e.content.error_code)) : shaka.Player.isBrowserSupported() ? (initPlayer(), initYoubora()) : console.error("Browser not supported!")
	})
}
...
function initPlayer() {
	if (printLog("initPlayer:: "), apiInfo.profiles.hasOwnProperty("auto") ? defaultQuality = "auto": apiInfo.profiles.hasOwnProperty("hd") ? defaultQuality = "hd": apiInfo.profiles.hasOwnProperty("high") ? defaultQuality = "high": apiInfo.profiles.hasOwnProperty("low") && (defaultQuality = "low"), "live" == hpvars.videoType) initUrl = apiInfo.profiles[defaultQuality],
	...
	0 === hpvars.timeLimit && (seekBar.style.display = "none", currentTime.style.display = "none");
	else {
		initUrl = apiInfo.profiles[defaultQuality].Default,
		contentId = initUrl.split(":")[2].split("/")[3],
		void 0 !== apiInfo.subtitle && Object.keys(apiInfo.subtitle).length > 0 ? (defaultSubtitleLang = Object.keys(apiInfo.subtitle)[0], defaultSubtitle = apiInfo.subtitle[defaultSubtitleLang], initSubtitle = defaultSubtitle.replace("web_subtitle", "web_subtitle_cors")) : (apiInfo.subtitle = {
			auto: "https://api.ttml.mytvsuper.com/web_subtitle_default.php"
		},
		initSubtitle = "https://api.ttml.mytvsuper.com/web_subtitle_default.php"),
		defaultAudioLang = "Cantonese",
		hpvars.quality_label = apiInfo.profiles[defaultQuality].quality,
		subtitleList = apiInfo.subtitle;
		var e = document.getElementById("textLanguages");
		for (var t in subtitleList) {
			var i = document.createElement("option");
			i.value = t,
			i.innerHTML = langConvert(t),
			e.appendChild(i),
			t == defaultSubtitleLang && i.setAttribute("selected", !0)
		}
	}
	hpvars.video_stage = apiInfo.video_stage,
	"" !== apiInfo.disclaimer_image && (hpvars.showFlip = !0, $("#flipContainer img").attr("src", apiInfo.disclaimer_image), $("#flipContainer").show()),
	...
	player.getNetworkingEngine().registerRequestFilter(function(e, t) {
		e == shaka.net.NetworkingEngine.RequestType.LICENSE && (printLog("LA url: " + t.uris[0]), t.headers.SOAPAction ? t.headers["Content-Type"] = "text/xml": t.headers["Content-Type"] = "application/octet-stream", t.headers["X-Service-ID"] = "super", t.headers["X-Client-Platform"] = "html5", t.headers["X-User-Token"] = hpvars.token)
	}),
	loadPlayer()
}
function loadPlayer() {
	var e = initUrl,
	t = hpconfig.initLaUrlWv + contentId,
	i = hpconfig.initLaUrlPr + contentId,
	a = initSubtitle;
	...
	"msedge" === $.browser.name && $.browser.version < 70 && (n.drm.servers["com.microsoft.playready"] = i),
	player.configure(n),
	player.load(e, hpvars.startPosition).then(function() {
		console.log("The video has now been loaded!");
		var e = document.querySelector("video").play();
		void 0 !== e && e.then(e = >
		{
			hpvars.autoplay = !0,
			"live" === hpvars.videoType ? hpvars.showFlip ? ($("#giantPlayButtonContainer").css("display", "none"), video.volume = 0, video.pause(), showFlip()) : video.play() : "" === hpvars.adTag ? (printLog("ads undefined"), hpvars.showFlip ? (video.pause(), showFlip()) : video.play()) : ($("#giantPlayButtonContainer").css("display", "none"), video.pause(), requestAds())
		}).catch(e = >
		{
			hpvars.autoplay = !1,
			$("#giantPlayButtonContainer").css("display", "block"),
			"undefined" == typeof ads && (console.log("ads undefined"), hpvars.showFlip ? $("#giantPlayButtonContainer").show() : console.log("ads undefined and no flip"))
		}),
		"live" !== hpvars.videoType ? (player.addTextTrack(a, defaultSubtitleLang, "subtitle", "application/ttml+xml", null), player.setTextTrackVisibility(!0)) : 0 !== player.getTextTracks().length && player.setTextTrackVisibility(!0),
		updateTracks(),
		updateTextLanguages()
	}).catch(onError)
}
function licensePostProcessor(e, t) {
...
function updateTracks() {
	...
	for (player.getVariantTracks().forEach(function(e) {
		e.active && (bitrateKbps = Math.floor(e.bandwidth / 1e3))
	}); e.firstChild;) e.removeChild(e.firstChild);
	for (var t in qualityList) {
		var i = document.createElement("option");
		i.value = t,
		i.innerHTML = langConvert(t),
		e.appendChild(i),
		void 0 === selectedQulity && (selectedQulity = defaultQuality),
		t == selectedQulity && i.setAttribute("selected", !0)
	}
}
function updateAudioLanguages() {
	updateLanguageOptions("audio", document.getElementById("audioLanguages"), player.getAudioLanguages(), player.getVariantTracks())
}
function updateTextLanguages() {
	var e = document.getElementById("textLanguages");
	player.isLive() && updateLanguageOptions("text", e, player.getTextLanguages(), player.getTextTracks())
}
function updateLanguageOptions(e, t, i, a) {
	...
	if (i.length <= 0) {
		...
		o.textContent = langConvert("auto"),
		o.value = "auto",
		o.selected = "auto",
		t.appendChild(o)
	} else {
		...
		i.forEach(function(i) {
			...
			a.textContent = langConvert("audio" == e ? "au" + r: i),
			a.value = i,
			a.selected = i == n.language,
			"audio" == e && (selectedAudioLang = n.language),
			t.appendChild(a),
			r++
		})
	}
}
function onAdaptation(e) {
	var t = document.getElementById("tracks");
	player.getVariantTracks().forEach(function(e) {
		if (e.active) for (var i = 0; i < t.options.length; ++i) {
			var a = t.options[i];
			if (a.value == e.id) {
				a.selected = !0;
				break
...
function refreshTextTrack() { ("live" !== hpvars.videoType || player.getTextTracks().length > 0) && (player.setTextTrackVisibility(!1), player.setTextTrackVisibility(!0))
}
function onTrackSelected(e) {
	var t, i = e.target,
	a = i.options[i.selectedIndex].value;
	printLog("onTrackSelected:: option=" + a),
	hpvars.changeSetting = !0;
	var n = document.getElementById("textLanguages"),
	o = n.selectedIndex >= 0 ? n.options[n.selectedIndex].value: "";
	if ("live" == hpvars.videoType) t = apiInfo.profiles[a];
	else for (var r in t = apiInfo.profiles[a].Default, hpvars.quality_label = apiInfo.profiles[a].quality, subtitleList) if (r == o) {
		subtitleUrl = subtitleList[r].replace("web_subtitle", "web_subtitle_cors");
		break
	}
	null !== t && "" !== t && (player.configure({
		preferredAudioLanguage: selectedAudioLang,
		preferredTextLanguage: o
	}), selectedQulity = a, player.load(t, video.currentTime).then(function() {
		"live" != hpvars.videoType ? player.addTextTrack(subtitleUrl, o, "subtitle", "application/ttml+xml", null) : refreshTextTrack(),
		video.play(),
		player.setTextTrackVisibility(!0)
	}).
	catch(onError))
}
function onAudioLanguageSelected(e) {
	var t = e.target,
	i = t.options[t.selectedIndex].value;
	printLog("onAudioLanguageSelected:: option=" + i),
	hpvars.changeSetting = !0,
	player.selectAudioLanguage(i),
	selectedAudioLang = i
}
function onTextLanguageSelected(e) {
	var t = e.target,
	i = t.options[t.selectedIndex].value;
	if (printLog("onTextLanguageSelected:: option=" + i), hpvars.changeSetting = !0, player.isLive()) player.selectTextLanguage(i);
	else {
		var a = initUrl;
		for (var n in subtitleList) if (n == i) {
			subtitleUrl = subtitleList[n].replace("web_subtitle", "web_subtitle_cors");
			break
		}
		if (null === subtitleUrl || "" === subtitleUrl) return;
		player.configure({
			preferredAudioLanguage: selectedAudioLang
		}),
		player.load(a, video.currentTime).then(function() {
			player.addTextTrack(subtitleUrl, i, "subtitle", "application/ttml+xml", null),
			video.play(),
			player.setTextTrackVisibility(!0)
		}).
		catch(onError)
...
function updateTime(e) {
	player.isLive() || isNaN(video.duration) || (document.getElementById("displayTime").innerHTML = toDateTime(video.duration, video.duration >= 3600))
}
function toDateTime(e, t) {
	var i = Math.floor(e / 3600),
	a = Math.floor(e / 60 % 60),
	n = Math.floor(e % 60);
	n < 10 && (n = "0" + n);
