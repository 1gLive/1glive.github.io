...
(function(){
	...
	epg.setDateTime = function(){
		...
		if(dd<10) {="" dd="0" +dd;="" }="" if(mm<10)="" mm="0" +mm;="" current_date="yyyy+mm+dd;" current_date_hyphen="yyyy+'-'+mm+'-'+dd;" };="" epg.setepgdata="function(){" current_network_code="$('#network_code').html();" $.ajax({="" url:="" "https:="" "+$('#api_domain').html()+"="" rest_listing_api="" epg="" format="" jsonp",="" datatype:="" "jsonp",="" data:="" network_code:"all",from:="" current_date},="" jsonp:="" "callback",="" success:="" function(rs){="" if(typeof="" rs.content[current_date_hyphen]="" !="undefined" ){="" data_epg="rs.content[current_date_hyphen];" epg.rendercurrentchannelepg(epg.getchannelepg(6,="" current_network_code));="" epg.prepareallepg();="" });="" epg.prepareallepg="function(){" var="" rs="$('#channel_listing').html();" epg.renderallepg(epg.preparechannelarray(json.parse(rs)));="" epg.rendercurrentchannelepg="function(epg){" wrap,="" time,="" programme;="" if(!$('.epg').hasclass('epg-loading')){="" $('.epg').html('');="" $('.epg').addclass('epg-loading');="" for(x="" in="" epg){="" wrap="$('<div/">').addClass('epg-row');
			time = $('<div>').addClass('epg-time').html(epg[x].time);
			programme = $('<div>').addClass('epg-programme').html(epg[x].programme);
	...
	epg.renderAllEPG = function(epg_all){
		...
		row_1.append(ch_name, time);
		right.append(row_1, programme_name);
		wrap.append(ch_logo, right, clearfix, cite_wrap);
		...
		for(x in epg_all){
			...
			if(typeof epg_all[x].current_programme[0] != 'undefined'){
				a.find('.epg-ch-name').html(a.find('.epg-ch-name').html()+' |');
			...
			for(depg in data_epg){
				if(data_epg[depg].network_code == network_code){
					for(y in data_epg[depg].epg){
						if(checkTime(data_epg[depg].epg[y].start_datetime) || data_epg[depg].epg.length == 1){
	...
	var checkTime = function(start_time){
		...
		var epg_time = new Date(start_time_split[1],parseInt(start_time_split[2], 10) - 1,start_time_split[3],start_time_split[4],start_time_split[5],start_time_split[6]);		var epg_ts = epg_time.getTime();
	...
	var processOnAirCodes = function(code){
		...
		var filters = [];
		for(var i = 0; i<=9 ;="" i++){="" filters.push('s'+i);="" }="" for(var="" i="0;" i<="9" filters.push('d'+i);="" var="" code_split="code.split(',');" same_count="0;" for(z="" in="" code_split){="" if(filters.indexof(code_split[z])="" !="-1){" same_count++;="" if(same_count="">= 2){
...
</=9></cite></div></div></div></div></div></div></div></div></div></a></cite></div></div></10)>